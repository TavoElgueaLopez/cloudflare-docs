...
:::note
The default shard count will be set to `auto` in the future, to enable automatic horizontal scaling.
:::
...
### Example
Your workload might require making 5,0000 requests per second to a pipeline. This workload requires files output files to be generated every 1 second, so it is  If you create a pipeline with a single shard, all 5,000 requests will be routed to the same shard. If your pipeline has been configured with a maximum batch duration of 1 second, all 5,000 requests will be batched, and a single file will be delivered.

Increasing the shard count to 2 will double the number of output files. The 5,000 requests will be split into 2,500 requests to each shard. Each shard will batch the data independently, and thus create two output files.

## Why shouldn't I just set the shard count to the maximum?
...
Each shard is capable of handling approximately 10,000 requests per second, or ingesting 10 MB / s of data. Either factor might act as the bottleneck, so choose the shard count based on the higher number.

For example, if you estimate that you will ingest 70 MB / s, making 70,000 requests per second, setup a pipeline with 7 shards. However, if you estimate that you will ingest 70 MB / s while making 100,000 requests per second, setup a pipeline with 10 shards.
